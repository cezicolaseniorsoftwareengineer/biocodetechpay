...
def test_external_pix_creates_single_transaction(db, user_alice):
    ...
    assert sent_tx.status == PixStatus.PROCESSING  # deferred debit: awaiting webhook
    assert sent_tx.value == 200.00

    # Deferred-debit model: balance NOT debited at creation
    db.refresh(user_alice)
    assert float(user_alice.balance) == pytest.approx(1000.00, abs=0.01)
